...
[[nodiscard]]
EnvironmentTextureSerializationData buildTextureData(const CompiledEnvironment& environment)
{
    EnvironmentTextureSerializationData result;

    result.metadata.reserve(environment.textures.size());

    uint64_t globalMipOffset = 0;
    uint64_t globalDataOffset = 0;

    for (const auto& texture : environment.textures)
    {
        ...
        result.data.insert(result.data.end(), texture.data.begin(), texture.data.end());
...
bool CompiledEnvironmentBlobWriter::write(const CompiledEnvironment& environment,
                                          const std::filesystem::path& outputPath)
{
    ...
    writer.addTypedSection(core::BlobSectionType::EnvironmentInfo,
                           std::span<const formats::environment::EnvironmentInfo>(&environment.info, 1));

    //
    // textures
    //

    const auto textures = buildTextureData(environment);

    addTypedSection(writer, core::BlobSectionType::EnvironmentTextureMetadata, textures.metadata);

    addTypedSection(writer, core::BlobSectionType::EnvironmentTextureMipMetadata, textures.mipMetadata);

    if (!textures.data.empty())
    {
        writer.addSection(core::BlobSectionType::EnvironmentTextureData,

                          std::span<const uint8_t>(textures.data.data(), textures.data.size()));
    }
