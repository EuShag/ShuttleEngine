...
{
std::optional<CompiledScene> SceneCompiler::compile(const std::filesystem::path& scenePath,
                                                    const SceneCompilerOptions& options)
{
    ...
    auto importedScene = AssimpSceneImporter::import(scenePath);
    ...
    SceneTextureResolver::resolve(*importedScene, scenePath.parent_path(), options.textureResolverOptions);
    ...
    SceneTextureCompilerResult textures = SceneTextureCompiler::compile(*importedScene, options.textureCompilerOptions);
    ...
    GeometryBuildResult geometry = GeometryBuilder::build(*importedScene, options.geometryOptions);
    ...
    SceneGraphBuildResult graph = SceneGraphBuilder::build(*importedScene, geometry, materials);

    //
    // lighting
    //

    LightingBuildResult lighting = LightingBuilder::build(*importedScene);
