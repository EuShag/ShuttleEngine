...
namespace shuttle::assets::scene_compiler
{
    namespace
    {
        namespace texture_format =
            shuttle::assets::formats::texture;

        void markRequiredTexture(
    ...
    int32_t textureIndex)
        ...
        std::vector<uint8_t> collectRequiredTextures(
            const ImportedScene& scene)
        {
            std::vector<uint8_t> required;
        ...
        texture_compiler::TextureCompileOptions makeOptionsForTexture(
            ...
            const SceneTextureCompilerOptions& sceneOptions)
        {
            ...
            options.generateMips =
                sceneOptions.generateMips;

            options.flipY =
                sceneOptions.flipY;

            options.roughnessIsGloss =
                sceneOptions.roughnessIsGloss;

            switch (texture.semantic)
            {
                case texture_format::TextureSemantic::Albedo:
                case texture_format::TextureSemantic::Emissive:
                    options.format =
                        VK_FORMAT_BC7_SRGB_BLOCK;
                    break;

                case texture_format::TextureSemantic::Normal:
                    options.format =
                        VK_FORMAT_BC5_UNORM_BLOCK;
                    break;

                case texture_format::TextureSemantic::ORM:
                    options.format =
                        VK_FORMAT_BC7_UNORM_BLOCK;
                    break;

                case texture_format::TextureSemantic::IBL:
                    options.format =
        ...
        texture_compiler::TextureCompileOptions makeOrmOptions(
            const SceneTextureCompilerOptions& sceneOptions)
        {
            texture_compiler::TextureCompileOptions options{};
        ...
        std::optional<texture_compiler::CompiledTexture> compileImportedTexture(
            const ImportedTexture& texture,
            const SceneTextureCompilerOptions& sceneOptions)
        {
            const texture_compiler::TextureCompileOptions options =
                makeOptionsForTexture(
                    texture,
                    sceneOptions);
            ...
            switch (texture.sourceKind)
            {
                case ImportedTextureSourceKind::File:
                {
                    ...
                    return texture_compiler::TextureCompiler::compileFile(
                        texture.sourcePath,
                        options);
                }
                ...
                case ImportedTextureSourceKind::EmbeddedEncoded:
                {
                    ...
                    return texture_compiler::TextureCompiler::compileMemory(
                        texture_compiler::ImageData{
                            .pixels = texture.embeddedBytes.data(),
                            .size = texture.embeddedBytes.size()
                        },
                        texture.formatHint,
                        options);
                }
                ...
                case ImportedTextureSourceKind::EmbeddedRGBA:
                {
                    ...
                    return texture_compiler::TextureCompiler::compileFromRGBA(
        ...
        std::optional<RgbaImage> loadRgbaImageFromTexture(
            const ImportedTexture& texture)
        {
            switch (texture.sourceKind)
            {
                ...
                case ImportedTextureSourceKind::EmbeddedEncoded:
                {
                    ...
                    uint8_t* rawPixels =
                        stbi_load_from_memory(
                            texture.embeddedBytes.data(),
                            static_cast<int>(texture.embeddedBytes.size()),
                            &width,
                            &height,
                            &channels,
                            4);
        ...
        RgbaImage resizeOrFill(
            const std::optional<RgbaImage>& source,
        ...
        std::optional<texture_compiler::CompiledTexture> buildOrmTexture(
            const ImportedScene& scene,
            ...
            const SceneTextureCompilerOptions& sceneOptions)
        {
            ...
            {
                occlusionImage =
                    loadRgbaImageFromTexture(
                        scene.textures[static_cast<size_t>(
                            material.occlusionTexture)]);
            }
            ...
            auto includeSize = [&targetWidth, &targetHeight]
                (const std::optional<RgbaImage>& image)
                {
                 ...
                 ++pixel)
            {
                ...
                const uint8_t ao =
                    occlusion.pixels[offset + 0];

                uint8_t rough =
                    roughness.pixels[offset + 0];

                const uint8_t metal =
                    metallic.pixels[offset + 0];
            ...
            const texture_compiler::TextureCompileOptions ormOptions =
                makeOrmOptions(
                    sceneOptions);
        ...
        bool materialNeedsGeneratedOrm(
            const ImportedMaterial& material)
        {
            if (material.ormTexture != InvalidIndexI32)
            {
            ...
            return material.occlusionTexture != InvalidIndexI32 ||
                material.roughnessTexture != InvalidIndexI32 ||
                material.metallicTexture != InvalidIndexI32;
        }

        std::string makeGeneratedOrmName(
    ...
    SceneTextureCompilerResult SceneTextureCompiler::compile(
        ImportedScene& scene,
        const SceneTextureCompilerOptions& options)
    {
        ...
        {
            result.importedToCompiledTexture.resize(
                scene.textures.size(),
                InvalidIndexI32);
        ...
        if (options.generateOrmTextures)
        {
                 ...
                 ++materialIndex)
            {
                ...
                std::optional<texture_compiler::CompiledTexture> generatedOrm =
                    buildOrmTexture(
                        scene,
                        material,
                        options);
                ...
                generatedTexture.name =
                    makeGeneratedOrmName(
                        materialIndex,
                        material);

                generatedTexture.sourceKind =
                    ImportedTextureSourceKind::Generated;

                generatedTexture.semantic =
                    texture_format::TextureSemantic::ORM;

                generatedTexture.formatHint =
                    "generated-orm";

                const int32_t importedTextureIndex =
                    static_cast<int32_t>(
                        scene.textures.size());

                scene.textures.push_back(
                    std::move(generatedTexture));

                const int32_t compiledTextureIndex =
                    static_cast<int32_t>(
                        result.textures.size());

                result.textures.push_back(
                    std::move(*generatedOrm));

                result.importedToCompiledTexture.push_back(
                    compiledTextureIndex);

                material.ormTexture =
                    importedTextureIndex;
            }
        }
        ...
        const std::vector<uint8_t> requiredTextures =
            collectRequiredTextures(
                scene);
        ...
        for (size_t textureIndex = 0;
             ...
             ++textureIndex)
        {
            if (textureIndex >= requiredTextures.size() ||
                requiredTextures[textureIndex] == 0)
            {
                continue;
            }
            ...
            std::optional<texture_compiler::CompiledTexture> compiled =
                compileImportedTexture(
                    texture,
                    options);
            ...
            result.importedToCompiledTexture[textureIndex] =
                compiledIndex;
        }
