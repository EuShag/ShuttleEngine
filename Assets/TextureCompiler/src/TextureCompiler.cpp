...
namespace shuttle::assets::texture_compiler
{
    ...
    std::optional<CompiledTexture> TextureCompiler::compileFile(
        ...
        const TextureCompileOptions& options)
    {
        ...
        {
            std::cerr
                << "[TextureCompiler] Failed to map texture file: "
                << filePath
                << std::endl;
    ...
    std::optional<CompiledTexture> TextureCompiler::compileMemory(
        ...
        const TextureCompileOptions& options)
    {
        ...
        {
            std::cerr
                << "[TextureCompiler] KTX/KTX2 import is not implemented yet."
                << std::endl;
    ...
    std::optional<CompiledTexture> TextureCompiler::compileRGBA8(
        const ImageSizedData& imageData,
        const TextureCompileOptions& options)
    {
        if (!imageData.valid())
        {
        ...
        std::vector<uint8_t> currentMipPixels(
            imageData.pixels,
            imageData.pixels +
             ...
             ++mip)
        {
            ...
            const uint64_t mipOffset =
                static_cast<uint64_t>(
                    output.data.size());

            const uint64_t mipSize =
                static_cast<uint64_t>(
                    compressedLevel.size());
            ...
            output.mipMetadata.push_back(
                mipMetadata);

            output.data.insert(
                output.data.end(),
                compressedLevel.begin(),
                compressedLevel.end());
            ...
            if (mip + 1 < mipCount)
            {
                ...
                std::vector<uint8_t> nextMipPixels(
                    static_cast<size_t>(nextWidth) *
        ...
        output.metadata.mipCount =
            mipCount;

        output.metadata.layerCount =
            1;

        output.metadata.format =
            options.format;

        output.metadata.imageType =
            texture_format::ImageType::Image2D;

        output.metadata.imageViewType =
            texture_format::ImageViewType::View2D;

        output.metadata.mipTableOffset =
            0;

        return output;
    }

    std::optional<CompiledTexture> TextureCompiler::compileRGBA16F(
        std::span<const ImageSizedData> mipChain,
        const TextureCompileOptions& options,
        uint32_t layerCount,
        texture_format::ImageViewType imageViewType)
    {
        ...
        {
            std::cerr
                << "[TextureCompiler] RGBA16F supports only BC6H_UFLOAT."
                << std::endl;
        ...
        CompiledTexture output{};
        output.mipMetadata.reserve(
            mipChain.size());

        for (uint32_t mipLevel = 0;
             mipLevel < mipChain.size();
             ++mipLevel)
        {
            const ImageSizedData& mipImage =
                mipChain[mipLevel];
            ...
            const auto* mipPixels =
                reinterpret_cast<const uint16_t*>(
                    mipImage.pixels);

            const size_t layerElementCount =
                static_cast<size_t>(mipImage.width) *
                static_cast<size_t>(mipImage.height) *
                4u;
                 ...
                 ++layer)
            {
                ...
                std::vector<uint8_t> compressedLayer =
                    compressBlocks(
                        reinterpret_cast<const uint8_t*>(
                            layerPixels),
                        static_cast<int>(mipImage.width),
                        static_cast<int>(mipImage.height),
                        8,
                        CompressionType::BC6H);

                mipSize +=
                    static_cast<uint64_t>(
                        compressedLayer.size());

                output.data.insert(
                    output.data.end(),
                    compressedLayer.begin(),
                    compressedLayer.end());
            }
    ...
    std::optional<CompiledTexture> TextureCompiler::compileFromRGBA(
    ...
    std::optional<CompiledTexture> TextureCompiler::packORM(
        const std::optional<std::filesystem::path>& occlusionPath,
        ...
        const TextureCompileOptions& options)
    {
        ...
        return packORM(
            ImageData{
                .pixels =
                    occlusionMapping.empty()
                        ? nullptr
                        : reinterpret_cast<const uint8_t*>(
                            occlusionMapping.data()),

                .size =
                    occlusionMapping.size()
            },
            ImageData{
                .pixels =
                    roughnessMapping.empty()
                        ? nullptr
                        : reinterpret_cast<const uint8_t*>(
                            roughnessMapping.data()),

                .size =
                    roughnessMapping.size()
            },
            ImageData{
                .pixels =
                    metallicMapping.empty()
                        ? nullptr
                        : reinterpret_cast<const uint8_t*>(
                            metallicMapping.data()),

                .size =
                    metallicMapping.size()
            },
            options);
    }

    std::optional<CompiledTexture> TextureCompiler::packORM(
        const ImageData& occlusionData,
        const ImageData& roughnessData,
        ...
        const TextureCompileOptions& options)
    {
        ...
        const int targetHeight =
            std::max(
                ...
                std::max(
                    aoPixels ? aoHeight : 1,
                    std::max(
                        roughPixels ? roughHeight : 1,
                        metallicPixels ? metallicHeight : 1)));
        ...
        auto resizeOrFill = []
            (uint8_t* sourcePixels,
                ...
                int targetWidth,
                int targetHeight) -> std::vector<uint8_t>
            {
        ...
        std::vector<uint8_t> ormPixels(
            static_cast<size_t>(targetWidth) *
            static_cast<size_t>(targetHeight) *
            4u);

        const bool roughnessIsGloss =
            options.roughnessIsGloss;

        #pragma omp parallel for default(none) shared(targetWidth, targetHeight, aoBuffer, roughBuffer, metallicBuffer, ormPixels, roughnessIsGloss)
        for (int y = 0;
             y < targetHeight;
             ++y)
        {
                 ...
                 ++x)
            {
                ...
                const uint8_t ao =
                    aoBuffer[pixelOffset];

                uint8_t roughness =
                    roughBuffer[pixelOffset];

                const uint8_t metallic =
                    metallicBuffer[pixelOffset];
        ...
        TextureCompileOptions ormOptions =
            options;
    ...
    std::optional<CompiledTexture> TextureCompiler::importSTB(
        const uint8_t* data,
        size_t size,
        const TextureCompileOptions& options)
    {
        ...
        std::optional<CompiledTexture> result =
            compileRGBA8(
                ImageSizedData{
                    .pixels = rawPixels,
                    .width =
    ...
    std::optional<CompiledTexture> TextureCompiler::importDDS(
        const uint8_t* data,
        size_t size)
    {
        tinyddsloader::DDSFile dds;

        if (dds.Load(data, size) !=
            tinyddsloader::Result::Success)
        {
            return std::nullopt;
        }
             ...
             ++mip)
        {
                 ...
                 ++layer)
            {
                ...
                output.data.insert(
                    output.data.end(),
                    bytes,
                    bytes + imageData->m_memSlicePitch);
        ...
        {
            output.metadata.imageViewType =
                texture_format::ImageViewType::View2DArray;
        }
        else
        {
            output.metadata.imageViewType =
    ...
    std::vector<uint8_t> TextureCompiler::compressBlocks(
        ...
        CompressionType compressionType)
    {
        ...
        #pragma omp parallel for default(none) shared(blocksY, blocksX, width, height, bytesPerPixel, pixels, output, rowStride, compressionType)
        for (int blockY = 0;
             blockY < blocksY;
             ++blockY)
        {
    ...
    void TextureCompiler::renormalizeNormalMap(
    ...
    CompressionType TextureCompiler::toCompressionType(
        VkFormat format)
    {
        switch (format)
        {
            ...
            case VK_FORMAT_BC6H_UFLOAT_BLOCK:
                return CompressionType::BC6H;

            case VK_FORMAT_BC7_SRGB_BLOCK:
            case VK_FORMAT_BC7_UNORM_BLOCK:
            default:
                return CompressionType::BC7;
        }
    }

    std::string TextureCompiler::normalizeExtension(
