...
namespace shuttle::assets::core {
    BlobView BlobView::open(const std::filesystem::path& path) {
        ...
        view.m_containers = view.readSpan<ContainerReference>(
            view.m_header->containerTableOffset,
            view.m_header->containerCount);

        view.m_sections = view.readSpan<BlobSection>(
            view.m_header->sectionTableOffset,
            view.m_header->sectionCount);

        view.validateRange(
            view.m_header->stringTableOffset,
            view.m_header->stringTableSize);

        view.m_strings = StringTableView(
            view.m_data +
            view.m_header->stringTableOffset,
            view.m_header->stringTableSize);
    ...
    const BlobHeader& BlobView::header() const noexcept { return *m_header; }
    std::span<const BlobSection> BlobView::sections() const noexcept { return m_sections; }
    std::span<const ContainerReference> BlobView::containers() const noexcept { return m_containers; }
    StringTableView BlobView::strings() const noexcept { return m_strings; }
