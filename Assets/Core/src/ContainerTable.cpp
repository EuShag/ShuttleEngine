...
uint32_t ContainerTableBuilder::addExternalContainer(StringTableBuilder& strings, std::string_view path,
                                                     uint64_t guidLow, uint64_t guidHigh, uint32_t flags)
{
...
const std::vector<ContainerReference>& ContainerTableBuilder::entries() const noexcept
{
    return m_entries;
}
...
ContainerTableView::ContainerTableView(std::span<const ContainerReference> entries) : m_entries(entries)
{
}
...
const ContainerReference& ContainerTableView::operator[](uint32_t index) const
{
    ...
    {
        throw std::out_of_range("ContainerTableView: index out of range.");
    }
