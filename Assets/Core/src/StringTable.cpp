...
namespace shuttle::assets::core {
    StringTableBuilder::StringTableBuilder() { clear(); }
    ...
    const std::vector<uint8_t>& StringTableBuilder::bytes() const noexcept { return m_bytes; }

    bool StringTableBuilder::empty() const noexcept { return m_bytes.empty(); }
    ...
    StringTableView::StringTableView(const uint8_t* data, uint64_t size)
    ...
    const uint8_t* StringTableView::data() const noexcept { return m_data; }
    uint64_t StringTableView::size() const noexcept { return m_size; }
}