...
namespace shuttle_engine {
    ...
    vk::ResultValue<SwapchainResources> RetireController::updateSwapchainResources(
        ...
        resources::DeviceAllocator const &deviceAllocator, PbrRender const &pbrRender,
        uint32_t frameCount,
        SwapchainResources &&oldSwapchainResources) {

        auto [createNewSwapchainResult, newSwapchain] = createSwapchain(
                        swapchainContext,
                        swapchainExtent,
                        *oldSwapchainResources.swapchain.swapchain
                    );
        if (createNewSwapchainResult != vk::Result::eSuccess) return {createNewSwapchainResult, {}};
        auto [createNewRenderTargetsResult, newRenderTargets] = pbrRender.createRenderTargets(
            swapchainContext.device,
            deviceAllocator,
            newSwapchain.images,
            newSwapchain.extent
        );
        ...
        retiredSwapchains.emplace_back(
            std::move(oldSwapchainResources.swapchain.swapchain),
