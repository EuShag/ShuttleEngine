...
namespace shuttle
{
HostMeshData TerrainGeometryGenerator::createFromHeightMap(const TerrainProperties& props,
                                                           const Image1D16bit& heightMap)
{ // Изменил Image1D16bit на Image
    HostMeshData mesh;
    ...
    std::vector<glm::vec3> bitangentAccumulator(totalVertices,
                                                glm::vec3(0.0f)); // Добавляем аккумулятор для битангентов
    ...
    for (uint32_t y = 0; y < height; ++y)
    {
        for (uint32_t x = 0; x < width; ++x)
        {
            ...
            mesh.positions[idx] = glm::vec3(x * gridCellSizeX - props.worldSize.x / 2.0f, worldHeight,
                                            y * gridCellSizeY - props.worldSize.y / 2.0f);

            mesh.uvs[idx] = glm::vec2(u_sample * props.textureRepeatFactor.x, v_sample * props.textureRepeatFactor.y);
        }
    }
    ...
    for (uint32_t i = 0; i < totalVertices; ++i)
    {
        ...
        { // В случае, если тангент не был посчитан
