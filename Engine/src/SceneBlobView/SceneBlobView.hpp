...
class SceneBlobView
{
  public:
    ...
    static SceneBlobView open(const std::filesystem::path& path)
    {
        SceneBlobView view{};
        ...
        view.validateRange(view.m_header->bulkDataOffset, view.m_header->bulkDataSize);

        const auto& h = *view.m_header;

        view.m_textures = view.readSpan<format::TextureMetaData>(h.textureTableOffset, h.textureCount);

        view.m_textureMips =
            view.readSpan<format::TextureMipMetaData>(h.textureMipmapTableOffset, h.textureMipmapCount);

        view.m_materials = view.readSpan<format::MaterialInfo>(h.materialTableOffset, h.materialCount);

        view.m_meshes = view.readSpan<format::MeshHeader>(h.meshTableOffset, h.meshCount);

        view.m_sceneNodes = view.readSpan<format::SceneNode>(h.sceneGraphOffset, h.sceneNodeCount);

        view.m_nodeLevels = view.readSpan<format::NodeLevelRange>(h.nodeLevelRangeTableOffset, h.nodeLevelRangeCount);

        view.m_dirLights = view.readSpan<format::DirectionalLight>(h.dirLightTableOffset, h.dirLightCount);

        view.m_pointLights = view.readSpan<format::PointLight>(h.pointLightTableOffset, h.pointLightCount);

        view.m_spotLights = view.readSpan<format::SpotLight>(h.spotLightTableOffset, h.spotLightCount);

        view.validateOptionalRange(h.materialDataOffset, h.materialDataSize);

        view.validateOptionalRange(h.sceneNodeDataOffset, h.sceneNodeDataSize);

        view.validateOptionalRange(h.nodeLevelDataOffset, h.nodeLevelDataSize);

        return view;
    }

    [[nodiscard]] const format::SceneHeader& header() const noexcept { return *m_header; }

    [[nodiscard]] const uint8_t* data() const noexcept { return m_data; }

    [[nodiscard]] uint64_t size() const noexcept { return m_size; }

    [[nodiscard]] std::span<const format::TextureMetaData> textures() const noexcept { return m_textures; }

    [[nodiscard]] std::span<const format::TextureMipMetaData> textureMips() const noexcept { return m_textureMips; }

    [[nodiscard]] std::span<const format::MaterialInfo> materials() const noexcept { return m_materials; }

    [[nodiscard]] std::span<const format::MeshHeader> meshes() const noexcept { return m_meshes; }

    [[nodiscard]] std::span<const format::SceneNode> sceneNodes() const noexcept { return m_sceneNodes; }

    [[nodiscard]] std::span<const format::NodeLevelRange> nodeLevels() const noexcept { return m_nodeLevels; }

    [[nodiscard]] std::span<const format::DirectionalLight> dirLights() const noexcept { return m_dirLights; }

    [[nodiscard]] std::span<const format::PointLight> pointLights() const noexcept { return m_pointLights; }

    [[nodiscard]] std::span<const format::SpotLight> spotLights() const noexcept { return m_spotLights; }
    ...
    [[nodiscard]] std::span<const format::TextureMipMetaData> mipsForTexture(uint32_t textureIndex) const
    {
        ...
        const uint64_t mipIndex =
            (texture.mipTableOffset - m_header->textureMipmapTableOffset) / sizeof(format::TextureMipMetaData);
    ...
    [[nodiscard]] std::span<const uint8_t> mipBytes(const format::TextureMipMetaData& mip) const
    {
        return bytesAt(mip.dataOffset, mip.dataSize);
    }

  private:
    template <typename T> [[nodiscard]] std::span<const T> readSpan(uint64_t fileOffset, uint32_t count) const
    {
        static_assert(std::is_trivially_copyable_v<T>, "SceneBlobView only supports trivially copyable structs.");
    ...
    [[nodiscard]]
    std::span<const uint8_t> optionalBytes(uint64_t fileOffset, uint64_t byteSize) const
    {
        if (fileOffset == 0 || byteSize == 0) return {};
        return bytesAt(fileOffset, byteSize);
    }

    void validateOptionalRange(uint64_t fileOffset, uint64_t byteSize) const
    {
    ...
    void validateRange(uint64_t fileOffset, uint64_t byteSize) const
    {
